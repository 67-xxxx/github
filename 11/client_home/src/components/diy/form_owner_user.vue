<template>
	<div>
			<b-form-group v-if="$check_register_field('add','user_name','/owner_user/view')" id="input-group-${idx}" label="用户姓名" label-for="input-${idx}">
			<b-form-input id="input-${idx}" v-model="form['user_name']" type="text" placeholder="用户姓名" trim></b-form-input>
		</b-form-group>
				<b-form-group v-if="$check_register_field('add','user_gender','/owner_user/view')" id="input-group-${idx}" label="用户性别" label-for="input-${idx}">
...
<script>
	...
	export default {
		...
		data(){
			return {
				url_upload: "~/api/owner_user/upload?",
