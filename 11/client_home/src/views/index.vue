		            ...
		            <div class="col-12">
		                <div class="sw_no">
							...
							<div class="card_notice">
								<div class="notice_title">系统公告 <router-link to="notice/list">更多</router-link></div>
								<!-- 公告组件 -->
		...
		<div class="warp">
		    <div class="container">
		        <div class="row">
					...
					<!-- 推荐单车信息模块(开始) -->
		<div class="warp" v-if="$check_action('/single_car_information/list', 'get')">
		    <!-- 容器 -->
		    <div class="container">
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/single_car_information/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_single_car_information = !show_list_single_car_information">
		                        <div class="switch_box">
		                            <span v-if="show_list_single_car_information"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!-- 单车信息推荐列表组件 -->
		                <list_single_car_information v-if="show_list_single_car_information" :list="list_single_car_information" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_single_car_information.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_single_car_information" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_single_car_information" :key="i" @click="to_details('single_car_information',o,'single_car_information_id')">
		                            <td v-for="(oj,n) in fields_single_car_information">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
...
<script>
	...
	export default {
		...
		data() {
			return {
				...
				show_list_article: true,
							show_list_single_car_information: true,
				list_single_car_information: [],
				vm_arr_single_car_information: [
					"single_car_information_id",
							"management_serial_number",
							"name_of_single_vehicle",
							"single_vehicle_type",
							"cover_image",
							"rental_unit_price",
							"number_of_single_vehicles",
							"rental_standards",
							"owner_user",
							"single_car_details",
						],
				fields_single_car_information: [
										{
						key: "management_serial_number",
						label: "管理序号",
						type: "文本"
					},
													{
						key: "name_of_single_vehicle",
						label: "单车名称",
						type: "文本"
					},
													{
						key: "single_vehicle_type",
						label: "单车类型",
						type: "下寻"
					},
													{
						...
						type: "图片"
													...
													{
						key: "rental_unit_price",
						label: "租用单价",
						type: "数字"
					},
													{
						key: "number_of_single_vehicles",
						label: "单车数量",
						type: "数字"
					},
													{
						key: "rental_standards",
						label: "租用标准",
						type: "下拉"
					},
													{
		...
		methods: {
			...
			toToggle(){
				this.isAdmin = !this.isAdmin;
						...
						// 获取单车信息列表
			get_single_car_information() {
				let url = "~/api/single_car_information/get_list?";
										this.$get(url, {
									"page": 1,
					"size": 8
				}, (json) => {
					if (json.result) {
						this.list_single_car_information = json.result.list;
					}
				})
			},
		...
		mounted() {
						this.get_single_car_information();
									this.get_menu();
			this.get_slides();
			this.get_article();
			// this.get_link();
		...
		computed: {
					    list_table_single_vehicle_type() {
		        var list = this.list_single_car_information;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_single_car_information.map((o) => {
		                // 第二个中括号是对象的属性
