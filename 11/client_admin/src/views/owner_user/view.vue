		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				...
				<el-form-item label="用户姓名" prop="user_name">
							  ...
							  v-if="user_group === '管理员' || (form_sub['owner_user_id'] && $check_field('set','user_name')) || (!form_sub['owner_user_id'] && $check_field('add','user_name'))"></el-input>
				...
				<el-form-item label="用户性别" prop="user_gender">
						...
						v-if="user_group === '管理员' || (form_sub['owner_user_id'] && $check_field('set','user_gender')) || (!form_sub['owner_user_id'] && $check_field('add','user_gender'))">
...
<script>
	...
	export default {
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["owner_user_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?owner_user_id=" + form_sub["owner_user_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/owner_user/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/owner_user/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/owner_user/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/owner_user/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/owner_user/view','get');
